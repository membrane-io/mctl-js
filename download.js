...
const tmpDir = path.join(os.tmpdir(), `membrane-mctl-cache-${packageVersion}`);
...
const REPO = "membrane-io/mctl-releases";
...
function unzipUnix(zipPath, destinationDir) {
  return new Promise((resolve, reject) => {
    console.log("unzip", zipPath, "-d", destinationDir);
    const unzipProc = child_process.spawn(
      "unzip",
...
async function unzipMctl(zipPath, destinationDir) {
  if (zipPath.endsWith(".zip")) {
    if (isWindows) {
      await unzipWindows(zipPath, destinationDir);
    } else {
      await unzipUnix(zipPath, destinationDir);
    }
  } else if (zipPath.endsWith(".tar.gz")) {
    await untar(zipPath, destinationDir);
  } else {
    throw new Error("Unknown archive format: " + zipPath);
  }

  const expectedName = path.join(destinationDir, "mctl");
  ...
  throw new Error(
    `Expecting mctl or mctl.exe unzipped into ${destinationDir}, didn't find one.`
  );
}
...
module.exports = async (opts) => {
  ...
  const extension = ".zip";
  const assetName = ["mctl", opts.version, opts.target].join("-") + extension;
  ...
  try {
    const destinationPath = await unzipMctl(assetDownloadPath, opts.destDir);
    if (!isWindows) {
